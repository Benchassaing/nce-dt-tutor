...
const mockAnalytics = {
  ...
  avgProgress: 42,
  completionRates: [
    { topic: "Renewable Energy", rate: 78 },
    { topic: "Solar Power", rate: 65 },
    { topic: "Isometric Drawing", rate: 45 },
    { topic: "Material Properties", rate: 38 },
                  ...
                  <tbody>
                    {mockDocuments.map((doc) => (
                      <tr key={doc.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="p-3">
                          <Badge variant="outline" className={getTypeColor(doc.type)}>{doc.type}</Badge>
                        </td>
                        ...
                        <td className="p-3 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button variant="ghost" size="icon" onClick={() => handleProcess(doc.id)} disabled={processingDoc === doc.id || doc.status === "PROCESSING"}>
                              {processingDoc === doc.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <RotateCcw className="h-4 w-4" />}
                            </Button>
                ...
                <AnalyticsCard title="Student Activity" subtitle="This week">
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="text-2xl font-bold">{mockAnalytics.activeThisWeek}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Total Study Hours</span>
                      <span className="text-2xl font-bold">{mockAnalytics.totalStudyHours.toLocaleString()}h</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Avg Progress</span>
                      <span className="text-2xl font-bold">{mockAnalytics.avgProgress}%</span>
                    </div>
                  </div>
                </AnalyticsCard>
                            ...
                            <td className="p-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-gray-500">student{i + 1}@school.mu</p>
                                </div>
                              </div>
                            </td>
