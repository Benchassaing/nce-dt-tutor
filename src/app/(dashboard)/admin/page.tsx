...
export default function AdminPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) { case "COMPLETED": return <Badge variant="success">{status}</Badge>; case "PROCESSING": return <Badge variant="warning">{status}</Badge>; case "FAILED": return <Badge variant="destructive">{status}</Badge>; default: return <Badge variant="secondary">{status}</Badge>; }
  };
  const getTypeColor = (type: string) => {
    switch (type) { case "TEXTBOOK": return "bg-blue-100 text-blue-700"; case "EXAM_PAPER": return "bg-green-100 text-green-700"; case "MARKING_SCHEME": return "bg-purple-100 text-purple-700"; case "TEACHER_NOTES": return "bg-orange-100 text-orange-700"; default: return "bg-gray-100 text-gray-700"; }
  };
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div><h1 className="text-2xl font-bold text-gray-900">Admin Panel</h1><p className="text-gray-600">Manage content, monitor students, and configure the platform</p></div>
          <div className="flex gap-2"><Button variant="outline"><Plus className="h-4 w-4 mr-2" />Add Topic</Button><Button variant="outline"><Settings className="h-4 w-4 mr-2" />Settings</Button></div>
        </div>
          ...
          <TabsContent value="documents" className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-200">
                <th className="text-left p-3 font-medium text-gray-500">Document</th><th className="text-left p-3 font-medium text-gray-500">Type</th><th className="text-left p-3 font-medium text-gray-500">Year</th><th className="text-left p-3 font-medium text-gray-500">Status</th><th className="text-left p-3 font-medium text-gray-500">Chunks</th><th className="text-left p-3 font-medium text-gray-500">Uploaded</th><th className="text-right p-3 font-medium text-gray-500">Actions</th>
              </tr></thead><tbody>
                {mockDocuments.map(doc => <tr key={doc.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-3 text-gray-600">{doc.year || "—"}</td><td className="p-3"><Badge variant={doc.status === "COMPLETED" ? "success" : doc.status === "PROCESSING" ? "warning" : doc.status === "FAILED" ? "destructive" : "secondary"}>{doc.status}</Badge></td>
                  <td className="p-3 text-gray-600">{doc.chunks.toLocaleString()}</td><td className="p-3 text-gray-600">{doc.uploadDate}</td>
                  <td className="p-3 text-right"><div className="flex items-center justify-end gap-2">
                    ...
                    <Button variant="ghost" size="icon"><Eye className="h-4 w-4" /></Button><Button variant="ghost" size="icon"><Edit className="h-4 w-4" /></Button>
            ...
            <TabsContent value="topics" className="space-y-4">
              <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-gray-900">Manage Curriculum Topics</h3><Button><Plus className="h-4 w-4 mr-2" />Add Topic</Button></div>
              <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-200">
                <th className="text-left p-3 font-medium text-gray-500">Code</th><th className="text-left p-3 font-medium text-gray-500">Title</th><th className="text-left p-3 font-medium text-gray-500">Unit</th><th className="text-left p-3 font-medium text-gray-500">Difficulty</th><th className="text-left p-3 font-medium text-gray-500">Questions</th><th className="text-left p-3 font-medium text-gray-500">Flashcards</th><th className="text-right p-3 font-medium text-gray-500">Actions</th>
              </tr></thead><tbody>
                {mockTopics.map(topic => <tr key={topic.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="p-3 font-mono text-sm text-gray-900">{topic.code}</td><td className="p-3 font-medium text-gray-900">{topic.title}</td>
                  ...
                  <td className="p-3 text-gray-600">{topic.questions}</td><td className="p-3 text-gray-600">{topic.flashcards}</td>
                  <td className="p-3 text-right"><div className="flex items-center justify-end gap-2"><Button variant="ghost" size="icon"><Eye className="h-4 w-4" /></Button><Button variant="ghost" size="icon"><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="text-red-600"><Trash2 className="h-4 w-4" /></Button></div></td></tr>))}
              </tbody></table></div>
            </TabsContent>

            <TabsContent value="analytics" className="space-y-6">
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <AnalyticsCard title="Completion Rate" subtitle="By topic"><div className="space-y-3">{mockAnalytics.completionRates.map(item => <div key={item.topic} className="space-y-1"><div className="flex justify-between text-sm"><span>{item.topic}</span><span className="font-medium">{item.rate}%</span></div><Progress value={item.rate} className="h-1.5" /></div>)}</div></AnalyticsCard>

                <AnalyticsCard title="Student Activity" subtitle="This week"><div className="space-y-4">
                ...
                <AnalyticsCard title="Weakest Topics" subtitle="Need attention"><div className="space-y-2">{mockAnalytics.completionRates.sort((a,b)=>a.rate-b.rate).slice(0,5).map(item => <div key={item.topic} className="flex items-center justify-between p-2 bg-red-50 rounded"><span className="text-sm">{item.topic}</span><Badge variant="destructive" className="text-xs">{item.rate}%</Badge></div>)}</div></AnalyticsCard>
              </div>

              <Card><CardHeader><CardTitle>Quick Actions</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-3"><Button variant="outline"><BarChart2 className="h-4 w-4 mr-2" />Export Analytics</Button><Button variant="outline"><Users className="h-4 w-4 mr-2" />Student Report</Button><Button variant="outline"><FileText className="h-4 w-4 mr-2" />Question Bank</Button></CardContent></Card>
            </TabsContent>

            <TabsContent value="users" className="space-y-4">
              <Card><CardHeader><CardTitle>Student Management</CardTitle></CardHeader><CardContent>
                <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-200">
                  <th className="text-left p-3">Student</th><th className="text-left p-3">Level</th><th className="text-left p-3">Progress</th><th className="text-left p-3">Streak</th><th className="text-left p-3">Last Active</th><th className="text-left p-3">Weak Areas</th><th className="text-right p-3">Actions</th>
                </tr></thead><tbody>
                  {Array.from({ length: 10 }).map((_, i) => <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="p-3"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-medium">S{i + 1}</div><div><p className="font-medium">Student {i + 1}</p><p className="text-sm text-gray-500">student{i + 1}@school.mu</p></div></div></td>
                    ...
                    <td className="p-3 text-right"><Button variant="ghost" size="icon"><Eye className="h-4 w-4" /></Button></td></tr>))}
...
function AnalyticsCard({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return <Card><CardHeader><CardTitle className="flex items-center justify-between"><span>{title}</span><Badge variant="outline" className="text-xs">{subtitle}</Badge></CardTitle></CardHeader><CardContent>{children}</CardContent></Card>;
}